...
## 7. Phase 11 实测数据与基线校验 (2026-06-28)

在 Phase 11“稳定性、性能与体验提升”中，我们修复了观察器泄露，重构了 ETag 缓存策略并配置了 Playwright 浏览器网络测试。我们利用 `node scripts/perf-budget.mjs` 重新捕获了首屏 JS 基线体积，确认如下：

### 各页面首屏 JS 真实依赖体积对比 (Gzip)

| 页面 | Phase 10 (Gzip) | Phase 11 (Gzip) | 预算指标线 (Gzip) | 状态 |
|---|---|---|---|---|
| **首页 (Home)** | 1.80 kB | **1.96 kB** | <= 55 kB | ✅ 达标 |
| **时光轴 (校史走廊)** | 0.71 kB | **0.71 kB** | <= 45 kB | ✅ 达标 |
| **同学录 (人物长廊)** | 3.47 kB | **4.02 kB** | <= 95 kB | ✅ 达标 |
| **学生详情页 (档案展柜)** | 0.96 kB | **0.98 kB** | <= 145 kB | ✅ 达标 |

### 性能基线与体验审计总结

1. **缓存一致性**：成功恢复了公开 JSON API 的 ETag 304 重验证响应机制，减少了重复的完整下载流量。
2. **感知加载提速**：对不支持 `requestIdleCallback` 的浏览器使用了更为紧凑的延迟水合，且音频预加载采取惰性触发，有效缓解了详情页首屏载入时的网络带宽竞争压力。
