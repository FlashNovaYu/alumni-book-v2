...
test.beforeEach(async ({ page }) => {
  ...
  await page.route('**/api/direct-conversations/conversation-lisi/messages**', (route) => route.fulfill({ contentType: 'application/json', body: JSON.stringify({ success: true, data: { items: [{ id: 'direct-1', conversationId: 'conversation-lisi', senderSlug: 'lisi', recipientSlug: 'chen-yu-hao', body: '周末见。', createdAt: '2026-07-11T08:00:00.000Z' }], nextCursor: null } }) }))
  ...
  await page.route(/\/api\/classmates(?:\?.*)?$/, (route) => route.fulfill({ contentType: 'application/json', body: JSON.stringify({ success: true, data: [{ name: '李四', slug: 'lisi', avatarUrl: null }, { name: '测试同学', slug: 'chen-yu-hao', avatarUrl: null }] }) }))
})
...
test.describe('Classmate Account Center Flow', () => {
  ...
  test('navigates to self-edit from account center', async ({ page }) => {
    ...
    const birthdayPicker = page.locator('.editor-overlay .calendar-date-picker input[aria-label="选择日期"]')
    await expect(birthdayPicker).toBeVisible()
    await birthdayPicker.click()
    await expect(page.locator('.editor-overlay .calendar-popover')).toBeVisible()
