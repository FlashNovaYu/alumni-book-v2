...
describe('档案卡共享元素转场', () => {
  ...
  it('在返回人物长廊的文档交换前恢复原分页或检索页中的共享目标', () => {
    ...
    expect(roster).toContain('v-for="mate in paginatedClassmates"')
    expect(roster).toContain(':archive-id="archiveIdFor(mate)"')
    expect(roster).toContain('function archiveIdFor(mate: Classmate)')
