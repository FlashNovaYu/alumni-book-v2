...
test.describe('公开站点真实浏览器性能预算', () => {
  ...
  test('静态首页不加载 Vue runtime，低端 CPU 首屏 TBT 保持在 600ms 内', async ({ page }) => {
    ...
    expect(tbt, JSON.stringify(longTasks)).toBeLessThan(1800)
  ...
  test('名册和年度册导航保持可交互且切换时间在预算内', async ({ page }) => {
    ...
    await waitNavigationInteractive(page)
    expect(Date.now() - rosterStart).toBeLessThan(2500)
    await page.waitForLoadState('load')
    ...
    await page.waitForURL(/\/yearbook\/?$/, { waitUntil: 'commit' })
    await waitNavigationInteractive(page)
    expect(Date.now() - rosterStart2).toBeLessThan(2000)
    await page.waitForLoadState('load')
    const yearbookStart2 = Date.now()
    await page.goBack({ waitUntil: 'commit' })
    await expect(page).toHaveURL(/\/roster\/?$/)
    await waitNavigationInteractive(page)
    expect(Date.now() - yearbookStart2).toBeLessThan(1200)
    await page.waitForLoadState('load')
  })
