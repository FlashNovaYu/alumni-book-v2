...
describe('Playwright preview isolation', () => {
  it('uses a private free port instead of stopping an existing local server', () => {
    ...
    expect(runner).toContain("require.resolve('astro/package.json')")
    expect(runner).toContain("'bin', 'astro.mjs'")
    expect(runner).not.toContain("'node_modules', 'astro', 'astro.js'")
    expect(runner).not.toContain('releasePreviewPort')
    expect(config).toContain("process.env.PLAYWRIGHT_PORT ?? '4321'")
    expect(config).toContain('pnpm exec astro preview')
    expect(config).not.toContain('node_modules/astro/astro.js')
