...
test('班级空间在三种视口保持分区结构、横向轨道和无页面横向溢出', async ({ page }) => {
  ...
  for (const viewport of viewports) {
    ...
    await expect(page.locator('.timeline-rail-card').first()).toContainText('初见')
    ...
    const layout = await page.evaluate(() => {
      ...
      const uncontainedOverflow = overflow
        .filter(element => !element.closest('.class-space-section-nav, .album-rail-viewport, .timeline-rail'))
        .slice(0, 8)
        .map(element => `${element.className}:${Math.round(element.getBoundingClientRect().right)}`)
    ...
    if (viewport.name === 'mobile') {
      await expect(page.locator('.timeline-rail')).toHaveCSS('scroll-snap-type', 'x')
    }
