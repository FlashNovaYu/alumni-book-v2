...
test('命令行参数强制完整 SHA、staging 地址和至少三个保留版本', () => {
  assert.deepEqual(parseAtomicReleaseArgs([
    ...
    '--promote-api-hook', '/bin/promote', '--rollback-api-hook', '/bin/rollback', '--live-base-url', 'http://127.0.0.1',
  ]).command, 'rollback')
...
test('引用缺失静态 asset 时 staging smoke 失败且不切换', async () => {
  const paths = await fixture()
  const api = await startFakeCandidateApi(releaseSha)
  let switched = false
  try {
    await writeFile(join(paths.artifactDir, 'index.html'), '<script src="/assets/missing.js"></script>')
    await assert.rejects(() => atomicDeploySelfHosted({ releaseSha, artifactDir: paths.artifactDir, releasesRoot: paths.releasesRoot, livePath: paths.livePath, candidateApiBaseUrl: api.baseUrl, switchCurrent: () => { switched = true } }), /missing\.js.*状态码异常/)
    assert.equal(switched, false)
  } finally {
    await api.close()
    await rm(paths.root, { recursive: true, force: true })
...
test('API promotion 后公网 smoke 失败会恢复旧静态和旧 API', async () => {
  const paths = await fixture()
  const switches = []
  const apiEvents = []
  try {
    ...
    await assert.rejects(() => atomicDeploySelfHosted({
      ...
      promoteApi: async () => { apiEvents.push('promote') }, rollbackApi: async () => { apiEvents.push('rollback') },
      switchCurrent: ({ releaseDir }) => { switches.push(releaseDir) }, liveBaseUrl: 'http://live.test',
      liveSmoke: async () => { throw new Error('public smoke failed') },
    }), /public smoke failed/)
    assert.deepEqual(apiEvents, ['promote', 'rollback'])
    assert.deepEqual(switches, [join(paths.releasesRoot, releaseSha), join(paths.releasesRoot, oldReleaseShas[1])])
  } finally {
    await rm(paths.root, { recursive: true, force: true })
  }
})
